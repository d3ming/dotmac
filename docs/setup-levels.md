...
| --- | --- | --- |
| **Essential — every development Mac** | `Brewfile`: Git, GitHub CLI, GNU Stow, ripgrep, uv; `stow/home`: Zsh, Vim, Readline, EditorConfig, and safe Git defaults | Small, portable tools needed to get a repo, search code, run the active Python projects, and restore the shell. No GUI app or service is installed, and no account is signed in by this level. |
...
| **Optional — desktop** | `Brewfile.desktop`: Ghostty, Zed, Obsidian | Personal interface choices. They add app downloads and may require sign-in or preference review. |
...
| Medium | Zed keymap, theme, and selected settings; Ghostty preferences | Track reviewed, portable preferences in separate optional Stow packages. Exclude API keys, account state, workspace history, and backup copies. |
